...
def test_full_model_flow(request):
    ...
    modelhub = ModelHub()
    modelhub.read_data(path, start_period=0, end_period=1)
    # TODO: Check if this is ok
    # pyhub.data["config"]["performance"]["pressure"]["pressure_on"]["value"] = 1
    modelhub.data["config"]["solveroptions"]["solver"]["value"] = request.config.solver
    modelhub.data["config"]["reporting"]["save_summary_path"][
        "value"
    ] = request.config.result_folder_path
    modelhub.data["config"]["reporting"]["save_path"][
        "value"
    ] = request.config.result_folder_path
    ...
    # modelhub.add_technology("period1", "node1", ["TestTec_WindTurbine"])
...
#     pyhub.data["config"]["performance"]["pressure"]["pressure_on"]["value"] = 0
#     pyhub.data["config"]["solveroptions"]["solver"]["value"] = request.config.solver
#     pyhub.data["config"]["reporting"]["save_summary_path"][
#         "value"
#     ] = request.config.result_folder_path
#     pyhub.data["config"]["reporting"]["save_path"][
#         "value"
#     ] = request.config.result_folder_path
#
...
#         for n in N:
#             pyhub.data["config"]["optimization"]["typicaldays"]["N"]["value"] = n
#             pyhub.data["config"]["optimization"]["typicaldays"]["method"][
#                 "value"
#             ] = method
...
#             if (
#                 pyhub.data["config"]["optimization"]["typicaldays"]["N"]["value"]
#                 != 0
#             ):
#                 pyhub.data._cluster_data()
#             if pyhub.data["config"]["optimization"]["timestaging"]["value"] != 0:
...
#     pyhub.data._read_model_config()
#
#     pyhub.data["config"]["optimization"]["timestaging"]["value"] = 4
#     pyhub.data["config"]["reporting"]["save_summary_path"][
#         "value"
#     ] = request.config.result_folder_path
#     pyhub.data["config"]["performance"]["pressure"]["pressure_on"]["value"] = 0
...
#
#     assert (
#         abs(npv_no_cluster - pyhub.model["full_resolution"].var_npv.value) / npv_no_cluster
#     ) <= 0.01
#
#     assert (
...
def test_objective_functions(request):
    ...
    modelhub.data["config"]["solveroptions"]["solver"]["value"] = request.config.solver
    modelhub.data["config"]["performance"]["pressure"]["pressure_on"]["value"] = 0
    modelhub.data["config"]["reporting"]["save_summary_path"][
        "value"
    ] = request.config.result_folder_path
    modelhub.data["config"]["reporting"]["save_path"][
    ...
    modelhub.construct_balances()
    modelhub._define_solver_settings()

    modelhub._optimize_emissions_net()
    modelhub._optimize_costs_minE()
    modelhub._optimize_costs_emissionslimit()

    modelhub._solve_pareto()
...
#     modelhub.read_data(path, start_period=0, end_period=1)
#
#     modelhub.data["config"]["scaling"]["scaling_on"]["value"] = 1
#     modelhub.data["config"]["reporting"]["save_summary_path"][
#         "value"
#     ] = request.config.result_folder_path
#     modelhub.data["config"]["reporting"]["save_path"][
