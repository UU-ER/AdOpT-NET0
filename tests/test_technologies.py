...
def test_electrolyzer(request):
    """
    tests Electrolyzer
    """
    time_steps = 1
    technology = "TestTec_Electrolyzer"
    tec = define_technology(
        technology, time_steps, request.config.technology_data_folder_path
    )
    ...
    # FEASIBILITY CASES
    model = construct_tec_model(tec, nr_timesteps=time_steps)
    model.test_const_output = Constraint(expr=model.var_output_tot[1, "hydrogen"] == 5)

    termination = run_model(model, request.config.solver)
    assert termination == TerminationCondition.optimal
    assert model.var_size.value == 5 / 0.655
    assert model.var_input_tot[1, "electricity"].value >= 5 / 0.655
    assert model.var_output_tot[1, "hydrogen"].value >= 10 * 0.5


def test_fuelcell(request):
    """
    tests FuelCell
    """
    time_steps = 1
    technology = "TestTec_FuelCell"
    tec = define_technology(
        technology, time_steps, request.config.technology_data_folder_path
    )
    ...
    model = construct_tec_model(tec, nr_timesteps=time_steps)
    model.test_const_output = Constraint(
        expr=model.var_output_tot[1, "electricity"] == 15
    )

    termination = run_model(model, request.config.solver)
    assert termination == TerminationCondition.optimal
    assert model.var_size.value == 15 / 0.5
    assert model.var_input_tot[1, "hydrogen"].value >= 15 / 0.5
    assert model.var_output_tot[1, "electricity"].value >= 30 * 0.5
