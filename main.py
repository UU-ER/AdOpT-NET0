...
topology.define_time_horizon(
    year=2001, start_date="01-01 00:00", end_date="01-01 10:00", resolution=1
)
...
if from_file == 1:
    ...
    lat = 52
    lon = 5.16
    data.read_climate_data_from_api(
        "onshore", lon, lat, save_path="./data/climate_data_onshore.txt"
    )
    lat = 52.2
    lon = 4.4
    data.read_climate_data_from_api(
        "offshore", lon, lat, save_path="./data/climate_data_offshore.txt"
    )
...
data.read_import_price_data(
    "onshore", "electricity", np.ones(len(topology.timesteps)) * 60
)
data.read_import_emissionfactor_data(
    "onshore", "electricity", np.ones(len(data.topology.timesteps)) * 0.1
)
