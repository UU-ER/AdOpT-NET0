...
# Typical days
configuration["optimization"]["typicaldays"]["N"]["value"] = 2
# Multi-year
...
with open(path / "period2" / "node_data" / "node1" / "Technologies.json", "r") as json_file:
    technologies = json.load(json_file)
technologies["new"] = ["Boiler_Small_NG", "Boiler_El"]

with open(path / "period2" / "node_data" / "node1" / "Technologies.json", "w") as json_file:
    json.dump(technologies, json_file, indent=4)
...
adopt.fill_carrier_data(path, value_or_data=50, columns=['Import limit'], carriers=['gas'], nodes=['node1'], investment_periods=["period1", "period2"])
adopt.fill_carrier_data(path, value_or_data=2, columns=['Demand'], carriers=['heat'], nodes=['node1'], investment_periods=["period1", "period2"])
adopt.fill_carrier_data(path, value_or_data=50, columns=['Import limit'], carriers=['electricity'], nodes=['node1'])
# Different gas and el prices in period 1 and 2
adopt.fill_carrier_data(path, value_or_data=50, columns=['Import price'], carriers=['gas'], nodes=['node1'], investment_periods=["period1"])
adopt.fill_carrier_data(path, value_or_data=100, columns=['Import price'], carriers=['gas'], nodes=['node1'], investment_periods=["period2"])
adopt.fill_carrier_data(path, value_or_data=50, columns=['Import price'], carriers=['electricity'], nodes=['node1'], investment_periods=["period1"])
adopt.fill_carrier_data(path, value_or_data=30, columns=['Import price'], carriers=['electricity'], nodes=['node1'], investment_periods=["period2"])
