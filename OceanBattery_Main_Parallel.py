...
def optimize_instance(energyhub, el_import_price_original, sd, capex_original, capex_share):
...
# topology.define_time_horizon(year=2001,start_date='05-01 00:00', end_date='05-01 23:00', resolution=1)
topology.define_time_horizon(year=2001,start_date='05-01 00:00', end_date='05-01 00:00', resolution=1)
...
def optimize_instance_wrapper(args):
    energyhub, el_import_price_original, sd, capex_original, capex_share = args
    optimize_instance(energyhub, el_import_price_original, sd, capex_original, capex_share)

# Generate all combinations of parameters
parameter_combinations = [(energyhub, el_import_price_original, sd, capex_original, capex_share)
                          for sd in SD_list
                          for capex_share in CAPEX_list]

# Use apply_async to parallelize the function calls
results = [pool.apply_async(optimize_instance_wrapper, (params,)) for params in parameter_combinations]

for result in results:
    result.get()

pool.map_async(optimize_instance_wrapper, parameter_combinations)
pool.close()
pool.join()