...
in "network_topology", e.g. my_case_study/period1/network_topology/new(or existing)/my_netw_type.
...
class Network(ModelComponent):
    ...
    def __init__(self, netw_data: dict):
        ...
        # General information
        self.connection = netw_data["connection"]
        self.distance = netw_data["distance"]
        if self.size_max_defined_per_arc:
            self.size_max_arcs = netw_data["size_max_arcs"]
    ...
    def fit_performance(self, modelhub: object, component_id: tuple, **kwargs):
        ...
        """
        super(Network, self).fit_performance(modelhub, component_id)
        time_independent = {}
    ...
    def construct_model(
        self, b_netw, modelhub, set_t_full, set_t_clustered, **kwargs
    ):
        """
        ...
        self.set_nodes = kwargs.get("set_nodes")

        self._define_possible_arcs(b_netw)

        if self.bidirectional_network:
            self._define_unique_arcs(b_netw)

        self._define_size(b_netw)
        self._define_capex_variables_netw(b_netw)
        self._define_opex_parameters(b_netw)
        self._define_emission_vars(b_netw)
        self._define_network_carrier(b_netw)
        self._define_inflow_vars(b_netw)
        self._define_outflow_vars(b_netw)
        self._define_energyconsumption_parameters(b_netw)
        ...
        def arc_block_init(b_arc, node_from, node_to):
            ...
            b_arc = self._define_capex_parameters_arc(
                b_arc, b_netw, node_from, node_to, modelhub
            )
            ...
            b_arc = self._define_opex_arc(b_arc, b_netw, modelhub)
            ...
            if b_arc.big_m_transformation_required:
                perform_disjunct_relaxation(b_arc)
        ...
        if self.bidirectional_network:
            self._define_bidirectional_constraints(b_netw)

        self._define_capex_total(b_netw)
        self._define_opex_total(b_netw, modelhub)
        self._define_inflow_constraints(b_netw)
        self._define_outflow_constraints(b_netw)
        self._define_emission_constraints(b_netw)

        self._define_energyconsumption_total(b_netw)
    ...
    def _define_capex_parameters_arc(
        self, b_arc, b_netw, node_from: str, node_to: str, modelhub
    ):
        """
    ...
    def _define_opex_arc(self, b_arc, b_netw, modelhub):
        """
    ...
    def _define_opex_total(self, b_netw, modelhub):
        """
    ...
    def write_results_netw_design(self, h5_group, model_block, config, modelhub):
        """
