...
from adopt_net0.core.components.utilities import set_capex_model
from adopt_net0.core.components.utilities import annualize, set_discount_rate, get_attribute_from_dict, \
...
class Stor(Technology):
    ...
    def fit_performance(self, modelhub: object, component_id: tuple, **kwargs):
        ...
        """
        super(Stor, self).fit_performance(modelhub, component_id)
        ...
        node = component_id[1]
        if (period, node, "TechnologyTimeSeries", self.name) in modelhub.data["time_series_data"]["full_resolution"]:
    ...
    def construct_model(self, b_tec, modelhub, set_t_full, set_t_clustered, **kwargs):
        ...
        super(Stor, self).construct_model(b_tec, modelhub, set_t_full, set_t_clustered, **kwargs)
        ...
        if "ambient_loss_factor" in coeff_td:
            ambient_loss_factor = coeff_td["ambient_loss_factor"]
        else:
            ambient_loss_factor = len(self.set_t_full) * [0.0]
    ...
    def _define_capex_parameters(self, b_tec, modelhub):
        ...
        :return: pyomo block with technology model
        """
        super(Stor, self)._define_capex_parameters(b_tec, modelhub)
    ...
    def _define_capex_variables(self, b_tec, modelhub):
        ...
        super(Stor, self)._define_capex_variables(b_tec, modelhub)
    ...
    def _define_capex_constraints(self, b_tec, modelhub):
        ...
        super(Stor, self)._define_capex_constraints(b_tec, modelhub)
