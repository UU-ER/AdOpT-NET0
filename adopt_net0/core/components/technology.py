...
from adopt_net0.core.components.utilities import annualize, set_discount_rate, link_full_resolution_to_clustered, \
    determine_variable_scaling, determine_constraint_scaling, get_attribute_from_dict, set_capex_model
...
class Technology(ModelComponent):
    ...
    def fit_performance(self, modelhub, component_id: tuple, **kwargs):
        ...
        """
        super(Technology, self).fit_performance(modelhub, component_id)
    ...
    def construct_model(self, b_tec, modelhub, set_t_full, set_t_clustered, **kwargs):
        ...
        if config["optimization"]["typicaldays"]["N"]["value"] == 0:
            ...
            if self.technology_model in technologies_modelled_with_full_res:
                ...
                self.set_t_performance = set_t_clustered
                self.sequence = modelhub.data["k_means_specs"]["sequence"]
        ...
        self._define_input_carriers(b_tec)
        self._define_output_carriers(b_tec)
        self._define_size(b_tec)
        self._define_capex_parameters(b_tec, modelhub)
        self._define_capex_variables(b_tec, modelhub)
        self._define_capex_constraints(b_tec, modelhub)
        self._define_input(b_tec, modelhub)
        self._define_output(b_tec, modelhub)
        self._define_opex(b_tec, modelhub)

        # EXISTING TECHNOLOGY CONSTRAINTS
        if self.existing and self.decommission == "only_complete":
            self._define_decommissioning_at_once_constraints(b_tec)
        ...
        if (config["optimization"]["typicaldays"]["N"]["value"] == 0) or (
            ...
            if self.technology_model in technologies_modelled_with_full_res:
                ...
                self._define_auxiliary_vars(b_tec, modelhub)
                if b_tec.find_component("var_input"):
                    self.input = b_tec.var_input_aux_low_res
                if b_tec.find_component("var_output"):
                    self.output = b_tec.var_output_aux_low_res

        self._define_emissions(b_tec)
        ...
        if config["performance"]["dynamics"]["value"]:
            ...
            if self.technology_model in technologies_modelled_with_dynamics:
                self._define_dynamics(b_tec, modelhub)
            else:
        ...
        modelhub.plugin_manager.emit(Hook.TECHNOLOGY_CONSTRUCTION_END,
                                     technology_constructor=self,
                                     modelhub=modelhub,
                                     b_tec=b_tec,
                                     )
    ...
    def _calculate_bounds(self):
        """
        Calculates bounds for variables (implemented in subclasses
        """
        pass
    ...
    def _define_capex_variables(self, b_tec, modelhub):
        """
    ...
    def _define_capex_parameters(self, b_tec, modelhub):
        """
    ...
    def _define_capex_constraints(self, b_tec, modelhub):
        """
    ...
    def _define_opex(self, b_tec, modelhub):
        """
    ...
    def _define_auxiliary_vars(self, b_tec, data: dict):
        ...
        if not (self.technology_model == "RES") and not (
            ...
            b_tec.var_input_aux_low_res = pyo.Var(
        ...
        b_tec.var_output_aux_low_res = pyo.Var(
            self.set_t_performance,
            b_tec.set_output_carriers,
            within=pyo.NonNegativeReals,
            bounds=init_output_bounds,
    ...
    def write_results_tec_operation(self, h5_group, model_block):
        ...
        if model_block.find_component("var_on_off"):
            h5_group.create_dataset(
                "var_x",
                data=[
                    0 if x is None else x
                    for x in [
                        model_block.var_on_off[t].value for t in self.set_t_performance
                    ]
                ],
            )
        if model_block.find_component("var_y"):
            h5_group.create_dataset(
                ...
                data=[
                    0 if x is None else x
                    for x in [
                        model_block.var_in_startup[t].value for t in self.set_t_performance
                    ]
                ],
            )
        if model_block.find_component("var_z"):
            h5_group.create_dataset(
                ...
                data=[
                    0 if x is None else x
                    for x in [
                        model_block.var_in_shutdown[t].value for t in self.set_t_performance
                    ]
                ],
            )
    ...
    # DYNAMICS FUNCTIONS
    def _define_dynamics(self, b_tec, modelhub):
        """
        ...
        # ):
        #     self._dynamics_SUSD_logic(b_tec)
        if not (self.performance_function_type == 4) and (SU_load + SD_load > -2):
            self._dynamics_fast_SUSD(b_tec)
    ...
    #         else:
    #             return (
    #                 b_tec.var_on_off[t] - b_tec.var_on_off[t - 1]
    #                 == b_tec.var_in_startup[t] - b_tec.var_in_shutdown[t]
    #             )
    #
    ...
    #         if t >= min_uptime:
    #             return b_tec.var_in_startup[t - min_uptime + 1] <= b_tec.var_on_off[t]
    #         else:
    #             return (
    #                 b_tec.var_in_startup[len(self.set_t_performance) + (t - min_uptime + 1)]
    #                 <= b_tec.var_on_off[t]
    #             )
    #
    ...
    #         if t >= min_downtime:
    #             return b_tec.var_in_shutdown[t - min_downtime + 1] <= 1 - b_tec.var_on_off[t]
    #         else:
    #             return (
    #                 b_tec.var_in_shutdown[len(self.set_t_performance) + (t - min_downtime + 1)]
    #                 <= 1 - b_tec.var_on_off[t]
    #             )
    #
    ...
    #             return (
    #                 sum(b_tec.var_in_startup[t] for t in self.set_t_performance) <= max_startups
    #             )
    #
    ...
    def _dynamics_fast_SUSD(self, b_tec):
        ...
        def init_SU_load(dis, t, ind):
            if ind == 0:  # no startup (y=0)
                dis.const_y_off = pyo.Constraint(expr=b_tec.var_in_startup[t] == 0)

            else:  # tech in startup
                dis.const_y_on = pyo.Constraint(expr=b_tec.var_in_startup[t] == 1)
        ...
        def init_SD_load(dis, t, ind):
            if ind == 0:  # no shutdown (z=0)
                dis.const_z_off = pyo.Constraint(expr=b_tec.var_in_shutdown[t] == 0)

            else:  # tech in shutdown
                dis.const_z_on = pyo.Constraint(expr=b_tec.var_in_shutdown[t] == 1)
