...
from adopt_net0.core.data_management.utilities import open_json, check_input_data_consistency, \
    merge_node_names_and_locations, get_temporal_information

log = logging.getLogger(__name__)


def read_topology(data_path: Path, start_period: int = None, end_period: int = None) -> dict:
    """
    Reads topology, matches node names and locations and adds temporal information

    :param Path data_path: Path to data
    :param int start_period: Initial period
    :param int end_period: Last period
    :return: Returns topology dict
    """
    with open(data_path / "Topology.json") as json_file:
        topology = json.load(json_file)

    # get node locations and map to nodes
    node_names = topology["nodes"]
    node_locations = pd.read_csv(
        (data_path / "NodeLocations.csv"), index_col=0, sep=";"
    )

    topology["nodes"] = merge_node_names_and_locations(
        node_names,
        node_locations)
...
def read_config(data_path: Path, topology: dict) -> dict:
    """
    Reads model configuration and nodal configurations

    :param Path data_path: Path to data
    :param dict topology: Topology dict
    :return: configuration dict
    :rtype: dict
    """
    # read global config
    with open(data_path / "ConfigModel.json") as json_file:
        config = json.load(json_file)

    # read nodal config
    config["node_config"] = {}
    for investment_period in topology["investment_periods"]:
        config["node_config"][investment_period] = {}
        for node in topology["nodes"]:
            ...
            config["node_config"][investment_period][node] = node_config

    return config


def read_time_series_data(data_path: Path, topology: dict) -> pd.DataFrame:
    """
    Reads all time-series data

    :param data_path: Path to data
    :param topology: Topology dict
    ...
    for investment_period in topology["investment_periods"]:
        for node in topology["nodes"]:
            ...
            # Technology Time Series Data
            var = "TechnologyTimeSeries"
            path = data_path / investment_period / "node_data" / node / "technology_time_series"
            if os.path.isdir(path):
                for file in (
                    path).iterdir():
                    if file.is_file() and file.suffix == ".csv":
                        name = file.stem  # filename without .csv
                        technology_time_series = pd.read_csv(file, sep=";", index_col=0).to_dict(orient="list")
                        for key in technology_time_series.keys():
                            data[(investment_period, node, var, name, key)] = (
                                replace_nan_in_list(technology_time_series[key])
                            )
...
def read_technology_data(data_path: Path, topology: dict) -> dict:
    """
    Reads all technology data

    :param data_path: Path to data
    :param topology: Topology dict
    :return: technology data from jsons (unprocessed)
    :rtype: dict
    """
    ...
    for investment_period in topology["investment_periods"]:
        ...
        for node in topology["nodes"]:
            ...
            for technology in technologies_at_node["new"]:
                load_path = data_path / investment_period / "node_data" / node / "technology_data"
            ...
            for technology in technologies_at_node["existing"]:
                load_path = data_path / investment_period / "node_data" / node / "technology_data"
                tec_data = open_json(technology, load_path)
                tec_data["name"] = technology + "_existing"
                tec_data["existing"] = 1
                tec_data["size"] = technologies_at_node["existing"][technology]
                technology_data[investment_period][node][technology + "_existing"] = tec_data

    return technology_data


def read_network_data(data_path: Path, topology: dict) -> dict:
    """
    Reads all network data

    :param data_path: Path to data
    :param topology: Topology dict
    :return: network data from jsons (unprocessed)
    :rtype: dict
    """
    network_data = {}

    # Loop through all investment_periods
    for investment_period in topology["investment_periods"]:
        network_data[investment_period] = {}

        # Get all networks in period
        with open(
                data_path / investment_period / "Networks.json"
        ) as json_file:
            networks = json.load(json_file)

        gammas = ["gamma1", "gamma2", "gamma3", "gamma4"]

        for networks_type in ["new", "existing"]:
            for network in networks[networks_type]:
                load_path = data_path / investment_period / "network_data"

                if networks_type == "new":
                    network_name = network
                    netw_data = open_json(network, load_path)
                    netw_data["name"] = network_name
                    netw_data["existing"] = 0
                    items = ["connection", "distance"]

                else:
                    network_name = network + "_existing"
                    netw_data = open_json(network, load_path)
                    netw_data["name"] = network_name
                    netw_data["existing"] = 1
                    items = ["connection", "distance", "size"]

                if netw_data["size_max_defined_per_arc"]:
                    items.append("size_max_arcs")

                for item in items:
                    try:
                        netw_data[item] = pd.read_csv(
                            data_path
                            / investment_period
                            / "network_topology"
                            / networks_type
                        ...
                        raise FileNotFoundError(f" {item}.csv file not found for network {network}")
                ...
                for gamma in gammas:
                    if netw_data["capex_defined_per_arc"]:
                        ...
                        try:
                            ...
                            raise FileNotFoundError(f" {gamma}.csv file not found for network {network}")

                network_data[investment_period][network_name] = netw_data

    return network_data


def read_system_data(data_path: Path, topology: dict) -> dict:
    """
    Reads system data (time series, technology data, network data)

    :param Path data_path: Path to data
    :param dict topology: Topology dict
    :return: dict with all system data
    :rtype: dict
    """
    time_series_data = read_time_series_data(data_path, topology)
    technology_data = read_technology_data(data_path, topology)
    network_data = read_network_data(data_path, topology)
...
class DataHandle:
    ...
    #     self.topology["time_index"]["full_resolution"] = time_index[
    #         self.start_period: self.end_period
    #     ]
    #     new_number_timesteps = len(self.topology["time_index"]["full_resolution"])
    ...
    #     self.topology["resolution_in_h"]["full_resolution"] = (
    #             pd.Timedelta(self.topology["time_index"]["full_resolution"].freq).seconds / 3600
    #     )
    #
    #     # Hours per day
    #     self.topology["hours_per_day"] = {}
    #     self.topology["hours_per_day"]["full_resolution"] = int(
    #         24 / self.topology["resolution_in_h"]["full_resolution"]
    #     )
    #
    #     # Log success
    ...
    #             var = "TechnologyTimeSeries"
    #             carrier = "global"
    #             technology_time_series = pd.read_csv(
    #                 self.data_path
    #                 / investment_period
    #                 / "node_data"
    #                 / node
    ...
    #                 index_col=0,
    #             ).to_dict(orient="list")
    #             for key in technology_data.keys():
    #                 data[(investment_period, node, var, carrier, key)] = (
    #                     replace_nan_in_list(technology_data[key])
    #                 )
    #
    #             # Carrier Data
    #             var = "CarrierData"
    ...
    #     data.index = self.topology["time_index"]["full_resolution"]
    #     data.columns.set_names(
    #         ["InvestmentPeriod", "Node", "Key1", "Key2", "Key3"], inplace=True
    #     )
    #     self.time_series["full_resolution"] = data
    #
    #     # Log success
    ...
    def _collect_full_res_data(self, investment_period: str) -> pd.DataFrame:
        ...
        """
        time_series = self.time_series["full_resolution"].loc[:, investment_period]
        ...
        if tec_series:
            ...
            tec_series.columns.set_names(
                ["Node", "Key1", "Key2", "Key3"], inplace=True
            )
    ...
    def _cluster_data(self):
        ...
        hours_per_day = self.topology["hours_per_day"]["full_resolution"]
    ...
    def _average_data(self):
        ...
        nr_timesteps_full = len(self.topology["time_index"]["full_resolution"])
        resolution_full = self.topology["resolution_in_h"]["full_resolution"]
        hours_per_day = self.topology["hours_per_day"]["full_resolution"]
