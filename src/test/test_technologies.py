...
def test_technology_RES_PV():
    ...
    data.technology_data["test_node1"]["Photovoltaic"].performance_data[
        "curtailment"
    ] = 0
    ...
    assert (
        ...
        .tech_blocks_active["Photovoltaic"]
        .var_size.value
    )
    assert (
        15
    ...
    for t in energyhub.model.set_t_full:
        energyhub.model.node_blocks["test_node1"].para_import_price[
            t, "electricity"
        ] = 0
    energyhub.solve()
    assert energyhub.solution.solver.termination_condition == "optimal"
...
def test_technology_RES_WT():
    ...
    configuration = ModelConfiguration()
    data.technology_data["test_node1"]["TestWindTurbine_Onshore_1500"].performance_data[
        "curtailment"
    ] = 0
    energyhub = EnergyHub(data, configuration)
    energyhub.configuration.reporting.save_path = "./src/test/results"
    ...
    assert (
        6
        == energyhub.model.node_blocks["test_node1"]
        .tech_blocks_active["TestWindTurbine_Onshore_1500"]
        .var_size.value
    )
...
def test_technology_CONV1():
    ...
    el_out_2 = round(
        energyhub.model.node_blocks["test_node1"]
        .tech_blocks_active[tecname]
        .var_output[2, "electricity"]
        .value,
        3,
    )
    assert (
        abs(tec_size * 10 * cost_correction - objective_value) / objective_value
        <= allowed_fitting_error
    )
    ...
    assert (
        abs(heat_out_2 / 0.8145 - hydrogen_in_2) / heat_out_2 / 0.8145
        <= allowed_fitting_error
    )
    ...
    assert 0 == gas_in_2
    assert (
        abs((heat_out_1 - 0.05) / 0.75 - hydrogen_in_1) / hydrogen_in_1
        <= allowed_fitting_error
    )
    assert (
        abs((heat_out_2 - 0.05) / 0.75 - hydrogen_in_2) / hydrogen_in_2
        <= allowed_fitting_error
    )
    assert 0.75 == heat_out_1
    assert 0 == el_out_1
    ...
    assert 0 == gas_in_2
    assert (
        abs((heat_out_1 - 0.25) / 0.5 - hydrogen_in_1) / hydrogen_in_1
        <= allowed_fitting_error
    )
...
def test_technology_CONV3():
    ...
    assert (
        abs(gas_in_1 - round(1 - 0.05 / 0.75, 3)) / round(1 - 0.05 / 0.75, 3)
        <= allowed_fitting_error
    )
...
def test_technology_OpenHydro():
    ...
    el_out_1 = round(
        energyhub.model.node_blocks["test_node1"]
        .tech_blocks_active["TestPumpedHydro_Open"]
        .var_output[1, "electricity"]
        .value,
        3,
    )
    ...
    data.node_data["test_node1"].data["climate_data"]["TestPumpedHydro_Open_inflow"][
        0
    ] = 0
    data.node_data["test_node1"].data["climate_data"]["TestPumpedHydro_Open_inflow"][
        1
    ...
    data.technology_data["test_node1"]["TestPumpedHydro_Open"].performance_data[
        "can_pump"
    ] = 0
...
def test_CAPEX_technologies():
    ...
    # collect data
    unitCAPEX = data.technology_data["test_node1"]["testCONV1_1"].economics.capex_data[
        "unit_capex"
    ]
    fixCAPEX = data.technology_data["test_node1"]["testCONV1_1"].economics.capex_data[
        "fix_capex"
    ]
    bp_x = data.technology_data["test_node1"]["testCONV1_1"].economics.capex_data[
        "piecewise_capex"
    ]["bp_x"]
    bp_y = data.technology_data["test_node1"]["testCONV1_1"].economics.capex_data[
        "piecewise_capex"
    ]["bp_y"]
...
def test_fast_dynamics():
    ...
    for j in CONV_Type:
        for i in perf_function_type:
            data_load_path = (
                r"./src/test/test_data/technology_dynamics_CONV"
                + str(j)
                + "_"
                + str(i)
                + ".p"
            ...
            if j != 3:
                ...
                tec_size = round(
                    energyhub1.model.node_blocks["test_node1"]
                    .tech_blocks_active[tecname]
                    .var_size.value,
                    3,
                )
                ...
                SU_number = sum(
                    ...
                    .var_y[i]
                    .value
                    for i in range(1, len(energyhub1.data.topology.timesteps) + 1)
                )
                ...
                assert (
                    round(
                        abs((gas_in_1 + hydrogen_in_1) - (gas_in_2 + hydrogen_in_2)), 3
                    )
                    <= tec_size / RT
                )
                ...
                hydrogen_in_6 = round(
                    energyhub2.model.node_blocks["test_node1"]
                    .tech_blocks_active[tecname]
                    .var_input[6, "hydrogen"]
                    .value,
                    3,
                )
                assert (
                    round(
                        abs((gas_in_5 + hydrogen_in_5) - (gas_in_6 + hydrogen_in_6)), 3
                    )
                    <= tec_size / RT
                )
                ...
                hydrogen_in_8 = round(
                    energyhub2.model.node_blocks["test_node1"]
                    .tech_blocks_active[tecname]
                    .var_input[8, "hydrogen"]
                    .value,
                    3,
                )
                assert (
                    round(
                        abs((gas_in_7 + hydrogen_in_7) - (gas_in_8 + hydrogen_in_8)), 3
                    )
                    <= tec_size / RT
                )

                # Test technology dynamic parameters: ramping rate operating only
                RT = 2
                data.technology_data["test_node1"][tecname].performance_data[
                    "ramping_time"
                ] = RT
                ...
                SUSD_check = sum(
                    ...
                    .var_x[t]
                    .value
                    for t in range(1, len(energyhub1.data.topology.timesteps) + 1)
                )

                assert SUSD_check <= len(energyhub1.data.topology.timesteps)
                ...
                # Test technology dynamic parameters: ramping rate
                RT = 1.5
                refsize = max(data.node_data["test_node1"].data["demand"]["heat"])
                data.technology_data["test_node1"][tecname].performance_data[
                    "ramping_time"
                ] = RT
                data.technology_data["test_node1"][tecname].performance_data[
                ...
                # Test technology dynamic parameters: ramping rate operation only
                RT = 1.2
                refsize = max(data.node_data["test_node1"].data["demand"]["heat"])
                data.technology_data["test_node1"][tecname].performance_data[
                    "ramping_time"
