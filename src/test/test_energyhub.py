...
def test_model2():
    ...
    # Size of Furnace
    size_res = (
        m.node_blocks["test_node1"].tech_blocks_active["Furnace_NG"].var_size.value
    )
    size_should = (
        max(data.node_data["test_node1"].data["demand"]["heat"])
        / data.technology_data["test_node1"][
            "Furnace_NG"
        ].fitted_performance.coefficients["heat"]["alpha1"]
    )
    assert round(size_res, 3) == round(size_should, 3)
    # Gas Import in each timestep
    import_res = [
        value(m.node_blocks["test_node1"].var_import_flow[key, "gas"].value)
        for key in m.set_t_full
    ]
    import_res = pd.Series(import_res)
    import_res = import_res.tolist()
    import_should = (
        data.node_data["test_node1"].data["demand"]["heat"]
        / data.technology_data["test_node1"][
            "Furnace_NG"
        ].fitted_performance.coefficients["heat"]["alpha1"]
    )
    import_should = import_should.tolist()
    assert [round(num, 3) for num in import_res] == [
        round(num, 3) for num in import_should
    ]
    # Total cost
    ...
    a = annualize(r, t, f)
    capex = (
        data.technology_data["test_node1"]["Furnace_NG"].economics.capex_data[
            "unit_capex"
        ]
        * size_res
        * a
    ...
    opex_fix = (
        capex * data.technology_data["test_node1"]["Furnace_NG"].economics.opex_fixed
    )
    opex_var = (
        sum(import_res)
        * data.technology_data["test_node1"]["Furnace_NG"].economics.opex_variable
    )
    ...
    emissions_should = (
        sum(import_res)
        * data.technology_data["test_node1"]["Furnace_NG"].performance_data[
            "emission_factor"
        ]
    )
...
def test_addtechnology():
    ...
    sizeBattery1 = (
        energyhub.model.node_blocks["test_node2"]
        .tech_blocks_active["Storage_Battery"]
        .var_size.value
    )
    assert 0 <= sizeWT1
    ...
    res = (
        energyhub.model.network_block["electricitySimple"]
...
def test_emission_balance1():
    ...
    configuration = ModelConfiguration()
    data.technology_data["onshore"]["Furnace_NG"].performance_data[
        "performance_function_type"
    ] = 1
    data.technology_data["onshore"]["Furnace_NG"].fitted_performance.coefficients[
        "heat"
    ]["alpha1"] = 0.9
    ...
    # network emissions
    emissionsNETW = sum(
        energyhub.model.network_block["electricityTest"].var_netw_emissions_pos[t].value
        for t in energyhub.model.set_t_full
    )
    emissionsFlowNETW = (
        sum(
            energyhub.model.network_block["electricityTest"]
            .arc_block[("onshore", "offshore")]
            .var_flow[t]
            .value
            for t in energyhub.model.set_t_full
        )
    ...
    assert (
        abs(
            sum(
                energyhub.model.node_blocks["onshore"]
                .tech_blocks_active["Furnace_NG"]
    ...
    assert (
        abs(
            sum(
                energyhub.model.node_blocks["onshore"].var_car_emissions_pos[t].value
                for t in energyhub.model.set_t_full
            )
            - import_emissions
        )
    ...
    # total emissions
    assert (
        abs(tec_emissions + import_emissions + emissionsNETW - emissionsTOT)
        / emissionsTOT
        <= 0.01
    )
...
def test_emission_balance2():
    ...
    configuration = ModelConfiguration()
    data.technology_data["test_node1"]["testCONV1_1"].performance_data[
        "emission_factor"
    ] = 1
    energyhub = ehub(data, configuration)
    energyhub.configuration.reporting.save_path = "./src/test/results"
