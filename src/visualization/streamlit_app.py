...
selected_option = st.sidebar.selectbox(
    "Select an option", ["Show single result", "Compare two Results"]
)
...
if st.session_state["path_loaded"] == 0:
    if selected_option == "Show single result":
        ...
        st.session_state["path"][1] = Path(
            st.sidebar.text_input("Enter path to results:", key="folder_key1")
        ...
        nr_pages = [1, 2]
        st.session_state["path"][1] = Path(
            st.sidebar.text_input("Enter path to results 1:", key="folder_key1")
            + "/optimization_results.h5"
        )
        st.session_state["path"][2] = Path(
            st.sidebar.text_input("Enter path to results 2:", key="folder_key2")
            + "/optimization_results.h5"
        )
        st.session_state["pages"] = [
            "Summary",
        ...
        try:
            for i in st.session_state["path"].keys():
                with h5py.File(st.session_state["path"][i], "r") as hdf_file:
                    st.session_state["nodes"][i] = extract_datasets_from_h5_dataset(
                        hdf_file["topology/nodes"]
                    )
                    st.session_state["carriers"].update(
                        extract_datasets_from_h5_dataset(hdf_file["topology/carriers"])
                    )
...
if st.session_state["path_loaded"] == 1:
    ...
    # Render side-bar
    if (selected_page == "Energy Balance at Node") or (
        selected_page == "Technology Operation"
    ):

        # Select a node
        selected_node = {}
        selected_node[1] = st.sidebar.selectbox(
            "Select a node:", st.session_state["nodes"][1], key="node_key1"
        )
        if len(st.session_state["path"]) == 2:
            selected_node[2] = st.sidebar.selectbox(
                "Select a node for second result:",
                st.session_state["nodes"][2],
                key="node_key2",
            )
    ...
    if selected_page == "Energy Balance at Node":
        ...
        selected_carrier = st.sidebar.selectbox(
            "Select a carrier:", st.session_state["carriers"]
        )
        ...
        for i in st.session_state["path"].keys():
            with h5py.File(st.session_state["path"][i], "r") as hdf_file:
                df = extract_datasets_from_h5_group(
                    hdf_file["operation/energy_balance"]
                )
                df = df.loc[
                    :,
                    (df.columns.get_level_values(0) == selected_node[i])
        ...
        if selected_option == "Show single result":
            export_csv(
                energybalance[1], "Download Energybalance as CSV", "energybalance.csv"
            )
        ...
        for i in st.session_state["path"].keys():
            plot_data = energybalance[i].loc[
                :,
                energybalance[i]
                .columns.get_level_values(2)
                .isin(selected_supply_series),
            ]
    ...
    # Technology Operation
        ...
        for i in st.session_state["path"].keys():
            tec_operation[i] = tec_operation[i].loc[
                :, (tec_operation[i].columns.get_level_values(1) == selected_tec)
            ]
        ...
        st.header("Input")
        variables_in = [
            col for col in tec_operation[1].columns if col.endswith("input")
        ]
        for i in st.session_state["path"].keys():
        ...
        st.header("Output")
        variables_out = [
            col for col in tec_operation[1].columns if col.endswith("output")
        ]
        for i in st.session_state["path"].keys():
        ...
        variables_other = [
            x
            for x in tec_operation[1].columns
    ...
    # Technology Sizes
        ...
        for i in st.session_state["path"].keys():
            technology_data_filtered[i] = technology_data[i][
                technology_data[i]["Variable"] == selected_var
            ]
        ...
        selected_technologies = st.multiselect(
            "Select Technologies to Filter", technologies, default=technologies
        )

        # Filter the DataFrame based on selected technologies
        for i in st.session_state["path"].keys():
            technology_data_filtered[i] = technology_data_filtered[i][
                technology_data_filtered[i]["Technology"].isin(selected_technologies)
            ]

        st.header(selected_var)
        for i in st.session_state["path"].keys():
            chart = (
                alt.Chart(technology_data_filtered[i])
                .mark_bar()
                .encode(y="Node:N", x="Value:Q", color="Technology:N")
                .interactive()
            )
    ...
    # Networks
        ...
        if st.session_state["network_keys"] is None:
            st.text(
                "Enter a path to a csv file that maps the node names to longitude and latitude."
            )
            network_key_path = Path(
                st.text_input(
                    "Enter file path to location keys of nodes:", key="network"
                )
            )
            if st.button("Load Node Locations"):
                st.session_state["network_keys"] = pd.read_csv(
                    network_key_path, sep=";", index_col=0
                )

        if st.session_state["network_keys"] is not None:
            # Load network design data
            with h5py.File(st.session_state["path"][1], "r") as hdf_file:
                network_design = extract_datasets_from_h5_group(
            ...
            network_design = network_design.pivot(
                columns="Variable", index=["Arc_ID", "Network"], values="Value"
            )
            ...
            selected_netw = st.sidebar.multiselect(
                "Select a network:", networks_available
            )
            network_df_filtered = network_design[
                network_design["Network"].isin(selected_netw)
            ]

            arc_ids = network_df_filtered[["Arc_ID", "FromNode", "ToNode"]]
            network_df_filtered = network_df_filtered.groupby("Arc_ID").sum()
            network_df_filtered.drop(
                columns=["FromNode", "ToNode", "Network"], inplace=True
            )
            ...
            if selected_graph in ["Network Size", "Network Capex", "Total Flow"]:
                ...
                for _, edge_data in network_df_filtered.iterrows():
                    ...
                    normalized_value = (
                        edge_data[variables[selected_graph]]
                    ) / max_value
                ...
                network_operation = network_operation.melt(
                    value_vars=network_operation.columns.tolist(), ignore_index=False
                )
                network_operation = network_operation.reset_index()
                network_operation.columns = [
                    "Timeslice",
                    "Network",
                    "Arc_ID",
                    "Variable",
                ...
                network_operation = network_operation[
                    network_operation["Variable"] == "flow"
                ]
                network_operation = network_operation.merge(arc_ids, on="Arc_ID")

                # Select a network
                network_operation_filtered = network_operation[
                    network_operation["Network"].isin(selected_netw)
                ]

                # Select type
                select_plotting = st.sidebar.selectbox(
                    "Select plotting type:", ["Energy Flow", "Line Utilization"]
                )
                ...
                network_operation_filtered = network_operation_filtered[
                    network_operation_filtered["Timeslice"] == t
                ]
                network_operation_filtered = (
                    network_operation_filtered.groupby(
                        ["Timeslice", "Arc_ID", "FromNode", "ToNode"]
                    )
                    .sum()
                    .reset_index()
                )
                ...
                for _, edge_data in network_operation_filtered.iterrows():
                    ...
                    flow_other_direction = network_operation_filtered[
                        (network_operation_filtered["FromNode"] == to_node_data.name)
                        & (network_operation_filtered["ToNode"] == from_node_data.name)
                    ]

                    uni_flow = (
                        flow_this_direction
                        - flow_other_direction.loc[:, "Value"].values[0]
                    )
                    ...
                    if uni_flow > 0.1:
                        if select_plotting == "Energy Flow":
                            ...
                            normalized_value = (
                                ...
                                / network_df_filtered[
                                    (
                                        network_df_filtered["FromNode"]
                                        == to_node_data.name
                                    )
                                    & (
                                        network_df_filtered["ToNode"]
                                        == from_node_data.name
                                    )
                                ]
                                .loc[:, "size"]
