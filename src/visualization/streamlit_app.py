...
# \\ad.geo.uu.nl\Users\StaffUsers\6574114\EhubResults\MES NorthSea\baseline_demand\20240213093347_Electrolysis_on/optimization_results.h5
...
if selected_option == 'Show single result':
    nr_pages = [1]
    st.session_state['path'][1] = Path(st.sidebar.text_input("Enter path to results:", key="folder_key1"))
    pages = ["Energy Balance at Node", "Technology Operation", "Technologies", "Networks"]

else:
    nr_pages = [1, 2]
    st.session_state['path'][1] = Path(st.sidebar.text_input("Enter path to results 1:", key="folder_key1"))
    st.session_state['path'][2] = Path(st.sidebar.text_input("Enter path to results 2:", key="folder_key2"))
...
# Networks
    ...
    # network_key_path = Path(st.sidebar.text_input("Enter file path to location keys of nodes:", key="network"))
    ...
    # Load Nodes
    if st.sidebar.button("Load Node Locations"):
