...
class Conv2(Technology):
        ...
        def init_SUSD_trajectories(dis, t, ind):
            if ind == 0:  # technology off
                ...
                def init_output_SU(const, car_output):
                    return self.output[t, car_output] == \
                           alpha1[car_output][0] * sum(self.input[t, car_input] for car_input in b_tec.set_input_carriers) + \
                ...
                def init_output_on(const, car_output):
                    return self.output[t, car_output] == \
                           alpha1[car_output][ind_bpx - 1] * sum(self.input[t, car_input] for car_input in b_tec.set_input_carriers) + \
    ...
    def define_ramping_rates(self, b_tec):
        ...
        :return:
        """
        super(Conv2, self).define_ramping_rates(b_tec)
