...
passing your input data folder path (which must be the same folder path as for the :ref:`Model Templates<data-preprocessing_create-model-templates>`).
...
- ``Topology.JSON``, in which you have :ref:`specified your system topology<data-preprocessing_define-topology>`.
...
- A folder for each investment period that you specified in the topology, containing:
    ...
    - ``Networks.JSON``, in which you specify the networks that are existing and that may be newly installed in the
      optimization. For each of the networks that you specify, an input data folder should be :ref:`added and filled <data-preprocessing_load-data>`
      in the corresponding folder (existing or new) in the network_topology folder.
    - A folder called ``network_data``, in which you :ref:`upload JSON files with network data <data-preprocessing_load-data>`
      for each network specified in the ``Networks.JSON``.
    - A folder called ``network_topology``, which itself contains:

        - A folder called ``existing``: containing the data templates that should be copied and :ref:`filled in<data-preprocessing_load-data>`
          for all existing network types.
        - A folder called ``new``: containing the data templates that should be copied and :ref:`filled in<data-preprocessing_load-data>`
    ...
    - A folder called ``node_data``, containing:
        ...
        - A folder for each node that you specified in the topology, containing:
            ...
            - ``Technologies.JSON``, in which you specify technologies that are existing and that may be newly installed
              ...
              :ref:`added and filled <data-preprocessing_load-data>` in the technology_data folder.
            - ``CarbonCost.csv``, in which you :ref:`specify carbon prices and subsidies<data-preprocessing_load-data>`
              for each timestep.
            - ``ClimateData.csv``, in which you :ref:`specify climate data <data-preprocessing_load-data>`
              for each timestep.
            ...
            - A folder called ``technology_data```, in which you :ref:`upload JSON files with technology data <data-preprocessing_load-data>`
              for each technology specified in the ``Technologies.JSON``.

If you have all the templates, you can now continue with :ref:`filling in the data <data-preprocessing_load-data>`
