...
Every rung of the ladder can be decomposed, so the algorithm can be tried on two nodes
before it is trusted on nine. What is decided and what is given is the ``--design`` of
``run.py``, with the same meaning, except that ``--from solved`` has nothing to refer to
here: the decomposition solves no reference run of its own.

Usage::

    python run_decomposed.py --rung 1L1S --hours 24
    python run_decomposed.py --rung 2L2S --design none --hours 168
    python run_decomposed.py --rung 3L3S --candidates 0.005,0.01,0.02,0.05 --workers 4
    python run_decomposed.py --rung 3L3S --input /scratch/case_copy --iterations 10
...
def parse_args(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Solves one rung of the ladder by a bilevel decomposition.",
        formatter_class=run.HelpFormatter,
    ...
    parser.add_argument(
        ...
        required=True,
        help="which rung is decomposed, i.e. which node set. Build it first with "
        "build.py",
    )
    ...
    parser.add_argument(
        ...
        "carries, anything else is a result folder of an earlier run",
    )
    parser.add_argument(
        "--given-type",
        choices=run.CANDIDATE_TYPES,
        default=run.DEFAULT_GIVEN_TYPE,
...
def main(argv=None):
    ...
    manifest = run.read_manifest(case_dir)
    if args.given_from == "solved":
        raise SystemExit(
            "--from solved has nothing to refer to here: the decomposition solves no "
            "reference run. Give it a result folder, or 'study'"
        )
    ...
    try:
        ...
        if not decide["tecs"]:
            pinned = run.given_design(case_dir, args.given_from, manifest["small"])
            design_backup = run.apply_design(
                input_path, pinned, manifest["small"], design_backup
            )
            print(
